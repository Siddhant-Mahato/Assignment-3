...
import style from "./DefferedList.module.css";
...
const DefferedList = () => {
  ...
  const deferredTasks = useSelector((state) =>
    state.tasks.filteredTasks.filter(
      (task) => task.status === "deferred" && !task.isHidden
    )
  );
  ...
  return (
    <div className={style.deff}>
      <div
        className={`${style.head} py-2 px-4 text-white font-semibold rounded-t-lg`}
      >
        Deffered
      </div>
      <div className={`${style.defferedList} overflow-y-auto mt-2`}>
